...
from metrics import simplicity
...
def evaluate_model(
    ...
    try:
        ...
        dataset_name = os.path.basename(dataset).removesuffix('.tsv.gz')
        setproctitle('-'.join([args.ALG, dataset_name, args.RANDOM_STATE]) + '@ZihanYu')
    except:
        pass
    ...
    if max_train_samples > 0 and len(y_train) > max_train_samples:
        ...
        if isinstance(X_train, pd.DataFrame):
            X_train = X_train.reset_index(drop=True).loc[sample_idx]
        else:
...
if __name__ == '__main__':
    ...
    # save_file = os.path.join(
    #     args.RDIR,
    #     '_'.join([dataset_name, args.ALG, str(args.RANDOM_STATE)])
    # )
    ...
    #         exit(0)
    #     else:
    #         print(f'overwriting existing results in {save_file}')
