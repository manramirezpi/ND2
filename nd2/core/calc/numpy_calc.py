...
def unpack_operands():
    def decorator(func):
        ...
        def wrapper(self, node, *args, **kwargs):
            operands = [self(op, **kwargs) for op in node.operands]
            with np.errstate(divide='ignore', invalid='ignore', over='ignore'):
                return func(self, node, *operands, *args, **kwargs)
        return wrapper
    return decorator
...
class NumpyCalc(Visitor):
    def __call__(self,
        ...
        try:
            ...
            raise ValueError(f'Error in NumpyCalc({node}): {e}') from e
