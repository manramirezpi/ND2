    ...
    def generate_leaf(self, nettype:Set[Literal['node', 'edge', 'scalar']]) -> Number|Variable:
        ...
        if const_range is not None:
            if nettype == 'scalar' or self.scalar_number_only or self._rng.integers(2):
                ...
                number = Number(self._rng.uniform(*const_range, (self.num_nodes,)), nettype='node')
            elif nettype == 'edge':
                number = Number(self._rng.uniform(*const_range, (len(self.edge_list[0]),)), nettype='edge')
            else:
                raise ValueError(f"Unknown nettype: {nettype}. Supported types are 'scalar', 'node', and 'edge'.")
            leafs = leafs + [number]
        
        return self._rng.choice(leafs)

    def generate_eqtree(self, nettype:Set[Literal['node', 'edge', 'scalar']]) -> Symbol:
