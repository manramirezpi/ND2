...
from ..core.symbols import (Symbol, Number, Variable, Add, Sub, Mul, Div, Max, Min,
...
class GP(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X:np.ndarray|pd.DataFrame|Dict[str,np.ndarray], y:np.ndarray|pd.Series):
        """
        Args:
            X: (n_samples, n_dims)
            y: (n_samples,)
        ...
        for iter in tqdm(range(1, 1+self.n_iter), disable=not self.use_tqdm):
            ...
            if self.n_jobs:
                population = Parallel(n_jobs=self.n_jobs)(delayed(self.evolve)(population, X, y, self.population_size // self.n_jobs) for _ in range(self.n_jobs))
            ...
            if not iter % self.log_per_iter or self.named_timer.total_time() > self.log_per_sec:
                log = {}
            ...
            record = {k: float(v) if isinstance(v, (np.float32, np.float64)) else v for k, v in record.items()}
            self.records.append(record)
            if self.save_path:
                with open(self.save_path, 'a') as f:
                    f.write(json.dumps(record) + '\n')
            if best.accuracy < 1e-6: 
                self.logger.info(f'Early stopping at iter {iter} with accuracy {best.accuracy} ({best})')
                break
    ...
    def predict(self, X:np.ndarray|pd.DataFrame|Dict[str,np.ndarray]) -> np.ndarray:
        ...
        if isinstance(X, np.ndarray):
            ...
            raise ValueError(f'Unknown type: {type(X)}')
        return self.eqtree.eval(vars=X, edge_list=self.edge_list, num_nodes=self.num_nodes)

    def evolve(self, population:List[Individual], X:Dict[str,np.ndarray], y:np.ndarray, children_size=None, elitism_k=None) -> List[Individual]:
        if children_size is None:
            children_size = self.population_size
        if elitism_k is None:
            elitism_k = self.elitism_k
        assert children_size > elitism_k, f'children_size {children_size} must be greater than elitism_k {elitism_k}'
        ...
        for parent in tqdm(self.tournament(population, children_size - elitism_k), disable=not self.use_tqdm):
            method = list(self.method_probs.keys())[np.searchsorted(np.cumsum(list(self.method_probs.values())), self._rng.random())]
            if method == 'crossover':
            ...
            if self._rng.random() < 0.1 and 0 < sum(isinstance(op, Number) for op in child.eqtree.preorder()) <= self.max_params:
                bfgs_fit = BFGSFit(child.eqtree, edge_list=self.edge_list, num_nodes=self.num_nodes)
                bfgs_fit.fit(X, y)
    ...
    def init_population(self, X:Dict[str,np.ndarray], y:np.ndarray) -> List[Individual]:
        population = []
    ...
    def tournament(self, population:List[Individual], num) -> List[Individual]:
        tournaments = self._rng.choice(population, size=(num, self.tournament_size), replace=True)
        winners = [max(tournament, key=lambda x:x.fitness) for tournament in tournaments]
        return winners
    ...
    def crossover(self, parent:Individual, donor:Individual) -> Individual:
        ...
        donored_subtree = self.get_random_subtree(donor, nettype=removed_subtree.replaceable_nettype())
    ...
    def subtree_mutation(self, parent:Individual) -> Individual:
        ...
        child.eqtree = child.eqtree.replace(subtree, self.generator.generate_eqtree(nettype=subtree.nettype))
        return child
    
    def hoist_mutation(self, parent:Individual) -> Individual:
        """Hoist mutation: 用某个子树替换根节点"""
        child = parent.copy()
        child.eqtree = child.eqtree.replace(child.eqtree, self.get_random_subtree(child, nettype=self.nettype))
        return child
    
    def point_mutation(self, parent:Individual) -> Individual:
        """Point mutation: 用随机符号替换 / 插入某个节点"""
        child = parent.copy()
        mutate_nodes = [node for node in child.eqtree.preorder() if self._rng.random() < self.p_point_replace]
        ...
        for node in mutate_nodes:
            if node.n_operands == 0:
                ...
                # else:
                #     sym = self._rng.choice(self.unary)(node)
            elif node.n_operands == 1:
                child_types = [op.nettype for op in node.operands]
                nodes = [sym for sym in self.unary if sym.possible_nettype(child_types) & node.replaceable_nettype()]
                sym = self._rng.choice(nodes)(*node.operands)
            elif node.n_operands == 2:
                # if self._rng.integers(0, 1):
                child_types = [op.nettype for op in node.operands]
                nodes = [sym for sym in self.binary if sym.possible_nettype(child_types) & node.replaceable_nettype()]
                assert len(nodes) > 0, f'No possible nettype for {node} with {child_types}'
                sym = self._rng.choice(nodes)(*node.operands)
                # else:
                #     sym = self._rng.choice(self.unary)(node)
            else:
    ...
    def set_fitness(self, individual:Individual, X:Dict[str,np.ndarray], y:np.ndarray) -> Individual:
        individual.complexity = len(individual.eqtree)
        self.named_timer.add('drop')
        y_pred = individual.eqtree.eval(vars=X, edge_list=self.edge_list, num_nodes=self.num_nodes)
        self.named_timer.add('evaluate.calc')
        individual.accuracy = float(np.mean((y_pred - y)**2))
        individual.fitness = float(0.999 ** individual.complexity / (1 + individual.accuracy / y.var()))
        if not np.isfinite(individual.fitness): individual.fitness = 0

    def get_random_subtree(self, individual:Individual|Symbol, nettype:Set[Literal['node', 'edge', 'scalar']]=None) -> Symbol:
        """
        follow the same approach as GPlearn and Koza (1992) to choose functions 90% of the time and leaves 10% of the time.
        """
        if isinstance(individual, Individual): individual = individual.eqtree
        if isinstance(nettype, str): nettype = {nettype}
        if nettype is None:
            nodes = [op for op in individual.preorder()]
        else:
            nodes = []
            for op in individual.preorder():
                if op.nettype in nettype or op.nettype == 'scalar':
                    nodes.append(op)
                elif op.nettype == 'edge' and 'node' in nettype:
                    if Aggr in self.unary: nodes.append(Aggr(op))
                    if Rgga in self.unary: nodes.append(Rgga(op))
                elif op.nettype == 'node' and 'edge' in nettype:
                    if Sour in self.unary: nodes.append(Sour(op))
                    if Targ in self.unary: nodes.append(Targ(op))
        if len(nodes) == 0:
            return self.generator.generate_eqtree(nettype=nettype)

        probs = np.array([0.9 if node.n_operands > 0 else 0.1 for node in nodes])
        subtree = nodes[np.searchsorted(np.cumsum(probs / probs.sum()), self._rng.random())]
        return subtree
